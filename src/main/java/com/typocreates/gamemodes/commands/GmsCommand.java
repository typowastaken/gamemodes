        ...
        if (strings.length == 0){
            if (commandSender instanceof Player) {
                ...
                if (p.getGameMode() != GameMode.SURVIVAL){
                    ...
                    p.sendMessage(ChatColor.RED + "Your gamemode hasn't changed since you're already in Survival.");
                }
            }else{
            ...
            if (target != null){
                if (target.getGameMode() != GameMode.SURVIVAL){
                    ...
                    if (commandSender instanceof Player) {
                        ...
                        p.sendMessage(ChatColor.AQUA + target.getDisplayName() + ChatColor.YELLOW + "'s gamemode has been set to Survival.");
                    }else{
                        Gamemodes.getPlugin().getLogger().info(ChatColor.AQUA + target.getDisplayName() + ChatColor.YELLOW + "'s gamemode has been set to Survival.");
                    }
                }else if (commandSender instanceof Player){
                    Player p = (Player) commandSender;
                    p.sendMessage(ChatColor.RED + "Nothing happened because the targeted player is already in Survival.");
                }else{
                    Gamemodes.getPlugin().getLogger().info(ChatColor.RED + "Nothing happened because the targeted player is already in Survival.");
                }
            }else if (commandSender instanceof Player){
                Player p = (Player) commandSender;
            ...
            Player p = (Player) commandSender;
            p.sendMessage(ChatColor.RED + "The maximum number of arguments is 1. Example: /gms <player>");
        }else{
            Gamemodes.getPlugin().getLogger().info(ChatColor.RED + "The maximum number of arguments is 1. Example: /gms <player>");
        }
        return true;
    }
}