    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (strings.length == 0) {
            if (commandSender instanceof Player ) {
                ...
                plugin.getLogger().info(ChatColor.RED + "You either have to be a player or target a player to use this command.");
            }
            ...
            if (target != null){
                if (target.getGameMode() != GameMode.ADVENTURE){
                    ...
                    if (commandSender instanceof Player) {
                        ...
                        plugin.getLogger().info(ChatColor.AQUA + target.getDisplayName() + ChatColor.YELLOW + "'s gamemode has been set to Adventure.");
                    }
                }else if (commandSender instanceof Player){
                    Player p = (Player) commandSender;
                    p.sendMessage(ChatColor.RED + "Nothing happened because the targeted player is already in Adventure.");
                }else{
                    plugin.getLogger().info(ChatColor.RED + "Nothing happened because the targeted player is already in Adventure.");
                }
            }else if (commandSender instanceof Player){
                Player p = (Player) commandSender;
                p.sendMessage(ChatColor.RED + "That player either does not exist or isn't currently online.");
            }else{
                plugin.getLogger().info(ChatColor.RED + "That player either does not exist or isn't currently online.");
            }
        }else if (commandSender instanceof Player){
            Player p = (Player) commandSender;
            p.sendMessage(ChatColor.RED + "The maximum number of arguments is 1. Example: /gma <player>");
        }else{
            plugin.getLogger().info(ChatColor.RED + "The maximum number of arguments is 1. Example: /gma <player>");
        }
        return true;
    }
}